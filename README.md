...
Each plate contains sealed cargo rooms in square, rectangular, and L-shaped layouts. Their interiors remain optically suppressed until the drone crosses a mechanical doorway, whose two leaves retract into the wall and remain open after the first traversal. The simple `DOSSIER [Q]` control beneath the subject telemetry, or the `Q` key, opens a physical mission dossier listing required freight by name and code, whether its room has been revealed, and whether it is missing, latched, or transferred. Some plates replace two manifested cargo cases with a mandatory storage-circuit order: two wall switches are distributed between separate storage rooms, and the marked extraction hatch remains sealed until both have been flipped with `E`. The dossier tracks circuit state and every stage of the personnel contract while safely pausing all plate clocks. Approaching a case opens a persistent prompt with its full cargo type and serial, and `E` latches it from the case tile or one connected neighboring tile. Decoy cargo can be inspected but not carried. Rooms are dressed with wall-hugging, non-blocking cabinets, tanks, manifolds, reels, work lights, salvage, and one guaranteed reclamation kiosk. Salvage enters a small sell inventory automatically when crossed. Standing on the kiosk and pressing `E` opens a safe counter where the player can buy limited repairs and one-hit Aegis wards, sell salvage, question the shopkeeper, or leave. Repairs bought at full integrity are banked and deploy automatically after a later hit. The kiosk accepts both saved account funds and field credits recovered during the current plate without counting spent field credits again in the final payout. A breach drops any carried cargo at the point of impact, while scattered credit chips are collected automatically by moving over them. Reaching a released extraction hatch ends the job even when cargo is missing, but speed, recovered cargo, restored circuits, loose credits, and breaches all affect the final pay.
...
The maze, revealed rooms, doors, cargo, circuit switches, survivor contract,
credits, and enemies are shared by the team. Drone integrity, equipped perks,
and shop protection remain per-player. A disconnected player gets a short
reconnect window. If the host does not return, the server elects the
longest-connected player, changes the authority epoch, and restores the newest
accepted checkpoint rather than allowing two peers to author the same run.
...
Camouflage, Ghost Form, and Hollow Killer share one physical `Space` channel, so only one can be fitted at a time. All other unlocked passive perks can be combined. Credits, achievement progress, equipped perks, and drone customization are retained alongside brightness, volume, resolution, and borderless-fullscreen settings in `%LOCALAPPDATA%\Dust\settings.json`.
...
- **Gameplay:** WASD/Arrow keys move one point; Q or the Dossier button opens the paused mission file, and Q/Esc closes it; E latches nearby manifested cargo, flips assigned storage switches, answers human distress prompts, returns escorted workers, or enters a kiosk while occupying its cell; Space activates the fitted active perk; loose credits and salvage collect automatically; R abandons and regenerates the current plate
